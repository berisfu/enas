...
class Model(object):
    def __init__(self,
        ...
        with tf.device("/cpu:0"):
            # training data
            self.num_train_examples = shapes['train']
            ...
            if datasets["valid"] is not None:
                ...
                self.x_valid_rl, self.y_valid_rl = datasets['valid_rl'][0], datasets['valid_rl'][1]
            #     if self.data_format == "NCHW":
            #         images["valid"] = tf.transpose(
            #             images["valid"], [0, 3, 1, 2])
                self.num_valid_examples = shapes['valid']
                self.num_valid_batches = (
                    (self.num_valid_examples + self.eval_batch_size - 1)
                    // self.eval_batch_size)
                    
                print('models valid_preprocess...')

            # test data
            self.x_test, self.y_test = datasets['test'][0], datasets['test'][1]
            # if self.data_format == "NCHW":
            #     images["test"] = tf.transpose(images["test"], [0, 3, 1, 2])
            self.num_test_examples = shapes['test']
