...
def get_train_ops(
    ...
    if optim_algo == "momentum":
        opt = tf.train.MomentumOptimizer(
            learning_rate*hvd_size, 0.9, use_locking=True, use_nesterov=True)
    elif optim_algo == "sgd":
        opt = tf.train.GradientDescentOptimizer(
            learning_rate*hvd_size, use_locking=True)
    elif optim_algo == "adam":
        opt = tf.train.AdamOptimizer(learning_rate*hvd_size, beta1=0.0, epsilon=1e-3,
                                     use_locking=True)
    else:
