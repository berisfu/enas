...
from src.skin5_placeholder.models import Model
...
from src.skin5_placeholder.image_ops import batch_norm_with_mask
from src.skin5_placeholder.image_ops import relu
from src.skin5_placeholder.image_ops import max_pool
from src.skin5_placeholder.image_ops import drop_path
...
class MicroChild(Model):
  def __init__(self,
               ...
               batch_size=16,
               clip_mode=None, # used to clip gradient for avoiding explosion
               grad_bound=None, # clip gradient rate
               l2_reg=1e-4,
               lr_init=0.1,
               lr_dec_start=0,
               lr_dec_every=10000,
               ...
               lr_max=None,
               lr_min=None,
               lr_T_0=None,   # question: the parameter for Temperature？
               lr_T_mul=None, # question: the parameter for Temperature？
               num_epochs=None,
               optim_algo=None,
               sync_replicas=False,
               num_aggregate=None,
    ...
    pool_distance = self.num_layers // 3 # do pooling operation evert pool_distance
  ...
  def _enas_cell(self, x, curr_cell, prev_cell, op_id, out_filters):
    ...
    out = [
        self._enas_conv(x, curr_cell, prev_cell, 3, out_filters), # separable_conv2d 3*3
        self._enas_conv(x, curr_cell, prev_cell, 5, out_filters), # separable_conv2d 5*5
        avg_pool,
        max_pool,
        x,
    ]
  ...
  def _enas_conv(self, x, curr_cell, prev_cell, filter_size, out_filters,
                 stack_conv=2):
    """
    Performs an enas separable convolution specified by the relevant parameters.
    """
  ...
  def _enas_layer(self, layer_id, prev_layers, arc, out_filters):
    ...
    layers = [prev_layers[0], prev_layers[1]]
    layers = self._maybe_calibrate_size(layers, out_filters, is_training=True) # make the shape layer[0] same to shape of layer[1]
    used = []
    for cell_id in range(self.num_cells):
      ...
      with tf.variable_scope("cell_{0}".format(cell_id)):
        with tf.variable_scope("x"):
          ...
          x_op = arc[4 * cell_id + 1]
          x = prev_layers[x_id, :, :, :, :]
          x = self._enas_cell(x, cell_id, x_id, x_op, out_filters) # Get the result of the op_id corresponding to the specified x_id
